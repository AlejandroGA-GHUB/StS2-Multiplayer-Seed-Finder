...
2. Card rewards past the second fight. Stops at 2 on purpose, since the early rooms are forced for everyone but anything after that depends on your route again. Will likely just ship the simple assuming its a hallway before a more in-depth approach.

3. The actual cards from Neow options that give you cards (Hefty Tablet, Arcane Scroll). It already accounts for the draws they use, it just won't show you what you'd
get. Nothing blocking this one, only work.

4. Shop cards and potions. The third relic slot is confirmed and solid, but the card side of a shop moves with how many shops you've visited and your carried relics can
rewrite the pool. Sits behind the route work too.

5. Linux and macOS launchers. Only the .bat files are Windows-only, the app itself already runs fine there. Small job if there's demand, and will likely be shipped regardless soon enough.
