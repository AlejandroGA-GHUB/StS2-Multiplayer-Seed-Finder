...
- **Three are blocked by the engine** — `PaelsClaw`, `PaelsGrowth`, `RoyalStamp`. Their vars read
  ...
  titles (`GOOPY.title` = "Goopy"), and the `LocString` even names the exact key. The one missing
...
### The multiplayer key
`Player.cs:330` — per-player seed is just the run seed plus lobby position:
```csharp
PlayerRng = new PlayerRngSet(
    StringHelper.GetDeterministicHashCode(seed) + (ulong)_runState.GetPlayerSlotIndex(this));
```
`GetPlayerSlotIndex` = `Players.IndexOf(player)` — plain lobby order. So P1 uses `hash(S)+0`, P2 `hash(S)+1`.
...
are rolled in `AfterObtained`, not at Neow. Which cards those are is still not modelled, but
**how many draws they cost is**, because they come off `PlayerRng.Rewards` and therefore shift
the first fight's reward — see `NeowRewardDrawCost` below.
...
if (i == State.Acts.Count - 1 && AscensionManager.HasLevel(AscensionLevel.DoubleBoss))
    act.SetSecondBossEncounter(
        UpFront.NextItem(act.AllBossEncounters.Where(e => e.Id != act.BossEncounter.Id)));
```
`AscensionLevel.DoubleBoss` is the 10th enum entry and `HasLevel` is `_level >= (int)level`,
so this is **A10+**. `AscensionManager.maxAscensionAllowed` is 10 — StS2 caps there, unlike
StS1's 20.
...
`MerchantInventory.PopulateRelicEntries` does not roll three rarities. It rolls two:
```csharp
new RelicRarity[3] { RollRarity(Player), RollRarity(Player), RelicRarity.Shop }
```
and `MerchantRelicEntry.FillSlot` fills each with `RelicFactory.PullNextRelicFromBack`, which
...
It does not. `RelicFactory.RollRarity` is a plain draw with fixed thresholds and no state at all:
```csharp
float num = rng.NextFloat();
return (num < 0.5f) ? Common : ((!(num < 0.83f)) ? Uncommon : Rare);
```
...
```csharp
RelicRarity rarity = RelicFactory.RollRarity(_rng);              // 1 NextFloat, RUN-level stream
RelicModel relic = _sharedGrabBag.PullFromFront(rarity, runState) // consumes NO rng
```
